...
class GeneralMotionRetargeting:
    ...
    #把预处理完的 human_data 赋值给每个 FrameTask 的目标位置/姿态。
    ...
    def retarget(self, human_data, offset_to_ground=False):
        # Update the task targets 设置任务目标
        ...
        if self.use_ik_match_table1:
            ...
            vel1 = mink.solve_ik(  #迭代求解IK
                self.configuration, self.tasks1, dt, self.solver, self.damping, self.ik_limits
            )
            self.configuration.integrate_inplace(vel1, dt) #更新位姿
            next_error = self.error1()
            num_iter = 0
            while curr_error - next_error > 0.001 and num_iter < self.max_iter: #循环直到误差不再下降
